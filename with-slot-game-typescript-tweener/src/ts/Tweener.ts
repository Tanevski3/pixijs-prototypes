...
export class Tweener {
  ...
  constructor(updater: Updater) {
    if (!updater) {
      throw new Error(
        "Updater function must be provided so the tween functions can be run"
      );
    }
...
export class Tweening {
  ...
  public constructor(updater: Updater) {
    if (!updater) {
      throw new Error(
        "Updater function must be provided so the tween functions can be run"
      );
  ...
  public start(finish?: Function, errorOccurred?: Function): void {
    this.shouldRun = true;
  ...
  public update = (): void => {
    if (this.shouldRun) {
      ...
      for (let i = 0; i < this.tweenOptions.length; i++) {
        ...
        if (!t.startTime) {
          throw Error("start time of tween must not be null");
        }

        const phase = Math.min(1, (now - t.startTime) / t.duration);
        ...
        if (
          ...
          t.object[t.property] = t.set(
            lerp(t.initialValue, t.finalValue as number, t.easing(phase))
          );
        } else {
  ...
  private static resolveOptionalValues(tweenOption: TweenOption): void {
    ...
    if (!tweenOption.startTime) {
      tweenOption.startTime = Date.now();
    } else {
      if (tweenOption.startTime < now) {
        throw Error(
...
/**
 * Basic lerp function. https://en.wikipedia.org/wiki/Linear_interpolation
